# RoMa
# Copyright roma.internal(c) 2020 NAVER Corp.
# CC BY-NC-SA 4.0
...
class _ProcrustesManualDerivatives(torch.autograd.Function):
    ...
    def backward(ctx, grad_R):
        ...
        Omega_klij = (USik_Vjl - USil_Vjk) * Internal._pseudo_inverse(DSk + DSl, gradient_eps)
...
def procrustes(M, force_rotation=False, regularization=0.0, gradient_eps=1e-5):
    ...
    """
    M, batch_shape = Internal.flatten_batch_dims(M, -3)
    R = _ProcrustesManualDerivatives.apply(M, force_rotation, regularization, gradient_eps)
    return Internal.unflatten_batch_dims(R, batch_shape)
...
def symmatrix_to_projective_point(A):
    ...
    """
    A, batch_shape = Internal.flatten_batch_dims(A, end_dim=-3)
    B, D1, D2 = A.shape
    assert (D1,D2) == (4,4), "Input should be a symmetric Bx4x4 matrix."
    eigenvalues, eigenvectors = Internal.symeig_lower(A)
    # Eigenvalues are sorted in ascending order
    q = eigenvectors[:,:,0]
    return Internal.unflatten_batch_dims(q, batch_shape)
...
def symmatrixvec_to_unitquat(x):
    ...
    Reference:
        V. Peretroukhin, M. Giamou, D. M. Rosen, W. N. Greene, N. Roy, and J. Kelly, “A Smooth Representation of Belief over SO(3) for Deep Rotation Learning with Uncertainty,” 2020, doi: 10.15607/RSS.2020.XVI.007.
    """
    x, batch_shape = Internal.flatten_batch_dims(x, end_dim=-2)
    ...
    A[:,2:,2] = x[:,7:9]
    A[:, 3,3] = x[:,9]
    return Internal.unflatten_batch_dims(symmatrix_to_projective_point(A), batch_shape)    
...
def rotvec_to_unitquat(rotvec):
    ...
    Returns:
        batch of unit quaternions (...x4 tensor, XYZW convention).
    """
    rotvec, batch_shape = Internal.flatten_batch_dims(rotvec, end_dim=-2)
    ...
    return Internal.unflatten_batch_dims(quat, batch_shape)
...
def unitquat_to_rotvec(quat, shortest_arc=True):
    ...
    """
    quat, batch_shape = Internal.flatten_batch_dims(quat, end_dim=-2)
    ...
    return Internal.unflatten_batch_dims(rotvec, batch_shape)
...
def rotmat_to_unitquat(R):
    ...
    Returns:
        batch of unit quaternions (...x4 tensor, XYZW convention).
    """
    matrix, batch_shape = Internal.flatten_batch_dims(R, end_dim=-3)
