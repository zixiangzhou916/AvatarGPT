...
import torch
import numpy as np
import networks.roma.internal as internal
import networks.roma.mappings as mappings

def is_torch_batch_svd_available() -> bool:
    """
    Returns True if the module 'torch_batch_svd' has been loaded. Returns False otherwise.
    """
    return internal._IS_TORCH_BATCH_SVD_AVAILABLE
...
def is_orthonormal_matrix(R, epsilon=1e-7):
    ...
    """
    R, batch_shape = internal.flatten_batch_dims(R, end_dim=-3)
...
def random_rotmat(size  = tuple(), dtype=torch.float, device=None):
    ...
    R = mappings.unitquat_to_rotmat(quat)
    return R
...
def random_rotvec(size = tuple(), dtype=torch.float, device=None):
    ...
    """
    quat = random_unitquat(size, dtype=dtype, device=device)
    return mappings.unitquat_to_rotvec(quat)
...
def rotvec_composition(sequence, normalize = False):
    ...
    """
    assert len(sequence) > 1, "Requiring at least two inputs."
    quats = [mappings.rotvec_to_unitquat(rotvec) for rotvec in sequence]
    q = quat_composition(quats, normalize=normalize)
    return mappings.unitquat_to_rotvec(q)
...
def rotmat_composition(sequence, normalize = False):
    ...
    for R in sequence[1:]:
        result = result @ R
    if normalize:
        result = mappings.special_procrustes(result)
    return result
...
def unitquat_slerp(q0, q1, steps, shortest_arc=True):
    ...
    rel_rotvec = mappings.unitquat_to_rotvec(rel_q, shortest_arc=shortest_arc)
    ...
    rots = mappings.rotvec_to_unitquat(rel_rotvecs.reshape(-1, 3)).reshape(*rel_rotvecs.shape[:-1], 4)
...
def unitquat_slerp_fast(q0, q1, steps, shortest_arc=True):
    ...
    Returns: 
        batch of interpolated quaternions (BxAx4 tensor).
    """
    q0, batch_shape = internal.flatten_batch_dims(q0, end_dim=-2)
    q1, batch_shape1 = internal.flatten_batch_dims(q1, end_dim=-2)
...
def rotvec_slerp(rotvec0, rotvec1, steps):
    ...
    """
    q0 = mappings.rotvec_to_unitquat(rotvec0)
    q1 = mappings.rotvec_to_unitquat(rotvec1)
    interpolated_q = unitquat_slerp(q0, q1, steps, shortest_arc=True)
    return mappings.unitquat_to_rotvec(interpolated_q)
...
def rotmat_slerp(R0, R1, steps):
    ...
    """    
    q0 = mappings.rotmat_to_unitquat(R0)
    q1 = mappings.rotmat_to_unitquat(R1)
    interpolated_q = unitquat_slerp(q0, q1, steps, shortest_arc=True)
    return mappings.unitquat_to_rotmat(interpolated_q)
...
def rigid_vectors_registration(x, y):
    ...
    R = mappings.special_procrustes(M)
    return R
