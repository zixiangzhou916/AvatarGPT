...
if __name__ == "__main__":
    ...
    parser.add_argument("--filedir", type=str, default="demo_outputs/demo/output/se1_p0/", help='motion npy file dir')
    parser.add_argument("--mesh_dir", type=str, default="demo_outputs/demo/meshes/se1_p0", help='motion npy file dir')
    parser.add_argument("--video_dir", type=str, default="demo_outputs/demo/videos/se1_p0/", help='motion npy file dir')
    ...
    for idx, filename in enumerate(filename_list):
        print('----', os.path.join(mesh_dir, filename+".npy"))
        print("[{:d}/{:d}] processed".format(idx+1, len(filename_list)))
        
        if os.path.exists(mesh_dir + filename + ".npy"):
            continue
        
        # if "B1320_T0000" not in filename: continue
        ...
        print("Caption: {:s}".format(captions))
        ...
        # vertices: [1, 6890, 3, T]
        vertices, faces = convert(pose_xyz[0].cpu().numpy(), outdir=mesh_dir, device_id=0, name=filename, pred=True)
        vertices = vertices.permute(0, 3, 1, 2) # [1, 6890, 3, T] -> [1, T, 6890, 3]
        
        output = {
            "pred": {"body": vertices[0].data.cpu().numpy()}, 
            "caption": data["caption"], 
            "color_labels": data.get("color_labels", None)
        }
        np.save(mesh_dir + filename + ".npy", output)
